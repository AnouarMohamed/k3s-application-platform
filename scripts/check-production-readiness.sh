...
if [[ "${REQUIRE_REAL_PRODUCTION_VALUES}" == "true" ]]; then
  ...
  if ! grep -Eq '^[[:space:]]*clusterIssuer:[[:space:]]+letsencrypt-production[[:space:]]*$' platform/settings.yaml; then
    echo "platform/settings.yaml must select letsencrypt-production before cutover." >&2
    exit 1
  fi
fi
