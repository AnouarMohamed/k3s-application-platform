...
require_secret GLPI_DB_PASSWORD
require_secret GLPI_DB_ROOT_PASSWORD
require_secret GLPI_BACKUP_PASSWORD
...
echo "Applying glpi-secret..."
kubectl -n apps create secret generic glpi-secret \
  --from-literal=GLPI_DB_PASSWORD="${GLPI_DB_PASSWORD}" \
  --from-literal=GLPI_DB_ROOT_PASSWORD="${GLPI_DB_ROOT_PASSWORD}" \
  --from-literal=GLPI_BACKUP_PASSWORD="${GLPI_BACKUP_PASSWORD}" \
  --dry-run=client -o yaml | kubectl apply -f -

echo "Applying glpi-secret in data namespace..."
kubectl -n data create secret generic glpi-secret \
  --from-literal=GLPI_DB_PASSWORD="${GLPI_DB_PASSWORD}" \
  --from-literal=GLPI_DB_ROOT_PASSWORD="${GLPI_DB_ROOT_PASSWORD}" \
  --from-literal=GLPI_BACKUP_PASSWORD="${GLPI_BACKUP_PASSWORD}" \
  --dry-run=client -o yaml | kubectl apply -f -
