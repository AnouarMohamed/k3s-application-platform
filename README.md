...
| Telemetry | Prometheus, Alertmanager, kube-state-metrics, node-exporter, Grafana, Loki, and Grafana Alloy |
...
| GLPI | admin | MariaDB PVC and `/var/glpi` PVC | Ingress allowlisted, DB dump and application-data backup coverage |
| Superset | admin | Superset home PVC | Ingress allowlisted, admin bootstrap secret documented for rotation |
| Passbolt | admin | MariaDB, GPG and JWT PVCs | DB and cryptographic-material restore runbook |
| Portainer | admin | Portainer data PVC | Optional UI; cluster-management RBAC is intentionally not granted by this baseline |
...
make check
make verify-images
kubectl kustomize . >/tmp/k3s-platform-rendered.yaml
```
...
- Loki stores logs.
- Grafana Alloy discovers pods through the Kubernetes API and forwards their logs to Loki without a hostPath log mount.
...
| `make verify-images` | Resolve every pinned image digest against its registry |
