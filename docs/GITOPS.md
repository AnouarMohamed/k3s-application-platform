...
spec:
  project: default
  source:
    repoURL: https://github.com/AnouarMohamed/k3s-application-platform.git
    targetRevision: main
    path: .
  destination:
...
1. `staging` for DNS, certs, backups, and upgrade rehearsals.
2. `production` after restore drills and rollback are proven.
