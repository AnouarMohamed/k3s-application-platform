...
This repository defines a production-oriented K3s platform for platform servers. It keeps the operational control of a small VPS-style deployment while adopting Kubernetes primitives that are useful in production: Deployments, Services, Ingress, Secrets, PVCs, probes, NetworkPolicy, Pod Security Admission labels, resource guardrails, and Git-friendly desired state.
